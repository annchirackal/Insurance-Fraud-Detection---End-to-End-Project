...
class KMeansClustering:
    ...
    def elbow_plot(self, data):
        """
            ...
            Args:
            Data: preprocessed data as pd.dataframe  
            Output: A picture saved to the directory and returns optimum number of clusters
        ...
        try:
            ...
            plt.ylabel('WCSS')
            #plt.show()
            fig_save_path='Clustering/K-Means_Elbow.PNG'
            directory = os.path.dirname(fig_save_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            plt.savefig(fig_save_path)  # saving the elbow plot locally
            
            # Log WCSS values
            self.logger_object.log(self.file_object, f"WCSS values: {wcss}")
            
            # finding the value of the optimum cluster programmatically
            self.kn = KneeLocator(range(1, 11), wcss, curve='convex', direction='decreasing')
            
            if self.kn.knee is None:
                self.logger_object.log(self.file_object, 'KneeLocator did not find an optimum number of clusters.')
                raise Exception('KneeLocator did not find an optimum number of clusters.')
    ...
    def create_clusters(self,data,number_of_clusters):
        """
            ...
            number_of_clusters(int): optimum number of clusters
    ...
    def run_clustering_data(self,data):
        self.data=data
        self.cluster_count=self.elbow_plot(self.data)
        self.clusterd_df= self.create_clusters(self.data,self.cluster_count)
        self.logger_object.log(self.file_object, 'Completed Clustering and exicted run_clustering')
        return self.clusterd_df
